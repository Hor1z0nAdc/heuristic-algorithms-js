...
onmessage = message => {
    tabuSearch(message.data.coords, message.data.polygonDeg, message.data.polygonCoords);
}

function tabuSearch(pointCoords, polygonDeg, polygonCoords) {
    ...
    do {
        ...
        let polygon;
        if(tabuIteration == 0) {
            polygon = Array.from(polygonCoords);
        }
        else {
            polygon = initPolygon2(polygonDeg, pointCoords);
        }
        tabuIteration++;
        iterations = 0;
...
function isTabu(polygon, polygonDeg) { 
    ...
    //iterates through the set of tabu polygons
    queue.forEach(tabuElement => { 
        let totalDist = 0;

        polygon.forEach(currentPoint => {
            //determine the closest point of tabu poly to current poly point
            const closestTabuPoint = closestPointToPoint(tabuElement, currentPoint);

            //add the distance from closest tabu poly point to current poly point
            let distSlice = distanceFromPoint(currentPoint, closestTabuPoint);
            totalDist += distSlice;
        })
...
function closestPointToPoint(polygon, point) {
    let closestPoint = polygon[0];
    let closestDist = distanceFromPoint(closestPoint, point)

    polygon.forEach(element => {
        let dist = distanceFromPoint(element, point)
...
function modifyPolygon(poly, index) {
    //returns the original point of polygons, except the one defined by the index
    //that one is modified by random value
...
//determines the centre of mass of a point set
...
function initPolygon2(polygonDeg, coords) {
    let poly = [];
    let degArea = 360 / polygonDeg;
    let midPoint = getCenter(coords);
    let furthestP = distanceFromPoint(midPoint, coords[0]);
    ...
    poly = Array.from(tempPolygonCoords);
    return poly;
}
