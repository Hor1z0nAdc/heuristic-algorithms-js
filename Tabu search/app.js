...
const runSpan = document.getElementById('run');
const optimalSpan = document.getElementById('optimalFit');
...
worker.onmessage = function (message) {
        ...
        const optFit = message.data.optimalFit;
        const tabuIterations = message.data.tabuIteration;
        ...
        optimalSpan.innerText = optFit;
        runSpan.innerText = tabuIterations;
}

function startAlgorithm() {
   worker.postMessage({coords, polygonDeg, polygonCoords});
}
...
function loadData() {
    ...
    reader.addEventListener("load", () => {
        ...
        ctx.clearRect(0, 0, canvasW, canvasH);
        polygonCoords = initPolygon2(polygonDeg, coords);
        drawPoints(coords, "green");
